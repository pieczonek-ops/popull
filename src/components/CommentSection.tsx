...
import { Send, Trash2, User, Reply, X, MessageSquare, MessageCircle } from 'lucide-react';
...
export default function CommentSection({ articleId, config }: CommentSectionProps) {
  ...
  return (
    <section className="mt-16 pt-12 border-t border-outline">
      {showCount && (
        <h3 className="text-2xl font-black mb-8 uppercase tracking-tight text-primary">Komentarze ({comments.length})</h3>
      )}
      
      {user ? (
        <div className={`bg-surface-container-low p-6 rounded-2xl border ${replyingTo ? 'border-primary/50 ring-1 ring-primary/20' : 'border-outline'} mb-10 transition-all`}>
          {replyingTo && (
        ...
        <div className="bg-surface-container-low p-10 rounded-3xl border border-outline text-center mb-10 shadow-xl">
      ...
      <div className="space-y-10">
        {topLevelComments.map((comment) => (
          <div key={comment.id} className="space-y-6">
            ...
            <div className="flex gap-4 group">
              {showAvatars && (
                <div className="w-11 h-11 rounded-full bg-surface-container flex-shrink-0 overflow-hidden flex items-center justify-center border border-outline">
                  {comment.userPhoto ? (
              ...
              <div className="flex-1">
                ...
                <p className="text-on-surface/90 text-[15px] leading-relaxed break-words bg-surface-container-low p-4 rounded-2xl rounded-tl-none border border-outline shadow-sm">
                  {comment.content}
                </p>
              </div>
            </div>
            ...
            {allowReplies && getReplies(comment.id).map((reply) => (
              <div key={reply.id} className="flex gap-4 ml-12 group">
                {showAvatars && (
                  <div className="w-9 h-9 rounded-full bg-surface-container flex-shrink-0 overflow-hidden flex items-center justify-center border border-outline">
                    {reply.userPhoto ? (
                ...
                <div className="flex-1">
                  ...
                  <div className="bg-surface-container-low p-4 rounded-2xl rounded-tl-none border border-outline shadow-sm relative">
        ...
        {comments.length === 0 && (
          <div className="text-center py-16 px-4 bg-surface-container-low/30 rounded-3xl border border-dashed border-outline">
            <MessageCircle className="w-12 h-12 text-on-surface/5 mx-auto mb-4" />
